...
(function () {
  ...
  async function changeRole(id){const role=prompt('New role: admin, teacher, or staff');if(!['admin','teacher','staff'].includes(role))return;const r=await sb.rpc('update_member_role',{p_member_id:id,p_role:role});if(r.error)return toast(r.error.message,'error');await audit('member_role_changed',{id,role});loadTeam();}
