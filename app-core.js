...
(function () {
  ...
  function renderAuth(mode='signin', message='') {
    $('auth-screen').innerHTML=`<div class="auth-shell"><div class="card auth-card">
  ...
  async function init(){
    setLoading(true); modalRoot=$('modal-root');
    if(!window.eduflowConfig?.supabaseUrl||!window.eduflowConfig?.supabaseKey){ showFatal('Configuration error','Supabase client configuration is missing.'); return; }
    if(!window.supabase?.createClient){ showFatal('Dependency error','Supabase could not be loaded. Refresh the page or check your network connection.'); return; }
    try{ sb=window.supabase.createClient(window.eduflowConfig.supabaseUrl,window.eduflowConfig.supabaseKey,{auth:{autoRefreshToken:true,persistSession:true,detectSessionInUrl:true}}); }
    catch(e){showFatal('Connection error',e.message);return;}
    const {data:{session:s}}=await sb.auth.getSession();
    if(s) await loadUser(s); else renderAuth(new URLSearchParams(location.search).get('mode')==='signup'?'signup':'signin');
    setLoading(false); setupListeners(); setupOffline();
  ...
  window.EduFlow={navigateTo,toast,closeModal};
