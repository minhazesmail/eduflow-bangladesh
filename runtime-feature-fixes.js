...
(function(){'use strict';
  ...
  async function queueAttention(studentId){if(demo){toast('Demo mode: notifications are disabled.','info');return;}const {client:s,session,orgId}=await getOrg();const {data:st,error:se}=await s.from('students').select('id,name,guardian_phone,guardian_name').eq('id',studentId).eq('organization_id',orgId).single();if(se||!st)throw new Error('Student not found.');if(!st.guardian_phone)throw new Error('Guardian phone is missing.');const {data:n,error}=await s.from('notifications').insert({organization_id:orgId,student_id:studentId,guardian_id:null,channel:'sms',type:'attendance_absent',title:'Attendance alert',body:`${st.name} has low recent attendance. Please check the EduFlow attendance details.`,status:'queued'}).select('id').single();if(error)throw error;try{await fetch(`${cfg.supabaseUrl}/functions/v1/dispatch-notification`,{method:'POST',headers:{Authorization:`Bearer ${session.access_token}`,'Content-Type':'application/json'},body:JSON.stringify({notification_id:n.id})});}catch(_){}toast('Guardian alert queued.','success');}
  ...
  function install(){
    wireDrafts();
    window.EduFlowRuntimeFixes={renderAttention,loading,queueAttention};
