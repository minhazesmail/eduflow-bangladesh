...
(function(){'use strict';
  ...
  async function open(){
    ...
    document.getElementById('pc-form').onsubmit=async e=>{e.preventDefault();const btn=e.submitter;btn.disabled=true;btn.textContent='Creating…';try{const provider=document.getElementById('pc-provider').value,studentId=document.getElementById('pc-student').value,amount=Number(document.getElementById('pc-amount').value),reference=document.getElementById('pc-reference').value.trim();const {data,error}=await s.functions.invoke('payment-gateway',{body:{provider,action:'create',amount,payer_reference:reference,merchant_invoice_number:`EDUFLOW-${studentId.slice(0,8)}-${Date.now()}`,callback_url:`${location.origin}/app.html?payment=return`}});if(error)throw error;if(data?.error)throw new Error(data.error);if(data?.checkout_url)window.open(data.checkout_url,'_blank','noopener,noreferrer');else if(data?.reference)toast(`${provider} checkout reference: ${data.reference}`,'success');else toast('Payment request created.','success');close();}catch(err){toast(err.message||'Could not create online payment.','error');}finally{btn.disabled=false;btn.textContent='Create checkout';}};
  }
