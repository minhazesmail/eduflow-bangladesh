...
create policy "owner admin manage fee invoices" on public.fee_invoices for all to authenticated
...
create policy "teacher staff read fee invoices" on public.fee_invoices for select to authenticated
using (organization_id = private.user_org_id() and private.user_role() in ('teacher','staff'));

create policy "owner admin staff manage fee payments" on public.fee_payments for all to authenticated
...
create policy "teacher read fee payments" on public.fee_payments for select to authenticated
using (organization_id = private.user_org_id() and private.user_role() = 'teacher');

create or replace function private.refresh_fee_invoice_status(p_invoice_id uuid)
returns void
language plpgsql
security definer
...
declare
  ...
  new_status text;
begin
  select greatest(amount_due - discount, 0) into due
  from public.fee_invoices where id = p_invoice_id;
  if due is null then return; end if;
  select coalesce(sum(amount), 0) into paid
  from public.fee_payments where invoice_id = p_invoice_id;
  if paid >= due then new_status := 'paid';
  elsif paid > 0 then new_status := 'partial';
...
create or replace function private.sync_fee_invoice_status()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  perform private.refresh_fee_invoice_status(coalesce(new.invoice_id, old.invoice_id));
...
create trigger fee_payment_status_sync
after insert or update or delete on public.fee_payments
for each row execute function private.sync_fee_invoice_status();
