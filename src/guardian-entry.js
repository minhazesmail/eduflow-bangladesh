...
(() => {
  ...
  const client = window.supabase.createClient(window.eduflowConfig.supabaseUrl, window.eduflowConfig.supabaseKey, { auth: { persistSession: true, autoRefreshToken: true, detectSessionInUrl: true } });
  ...
  const query = async (promise) => { const result = await promise; if (result.error) throw result.error; return result.data || []; };

  async function load() {
    try {
      const { data: { session } } = await client.auth.getSession();
      if (!session?.user) { root.innerHTML = '<div class="card"><h2>Sign in required</h2><p class="subtitle">Use the invitation link from your coaching center.</p><a class="btn btn-primary" href="app.html">Sign in</a></div>'; return; }
      ...
      if (accountError || !account) { root.innerHTML = '<div class="card"><h2>Portal not ready</h2><p class="subtitle">Ask your coaching center to resend the guardian invitation.</p></div>'; return; }
      ...
      const students = studentIds.length ? await query(client.from('students').select('id,name,student_code,class_level,monthly_fee,batches(name)').in('id', studentIds)) : [];
      const attendance = studentIds.length ? await query(client.from('attendance').select('student_id,present,attendance_date').in('student_id', studentIds).order('attendance_date', { ascending: false }).limit(300)) : [];
      const payments = studentIds.length ? await query(client.from('payments').select('student_id,amount,payment_method,receipt_no,paid_at').in('student_id', studentIds).order('paid_at', { ascending: false }).limit(100)) : [];
      const results = studentIds.length ? await query(client.from('results').select('student_id,marks,created_at,exams(name,total_marks)').in('student_id', studentIds).order('created_at', { ascending: false }).limit(100)) : [];
      ...
      const noticeHtml = notices.length ? notices.map((notice) => `<div class="portal-row"><span><strong>${escapeHtml(notice.title)}</strong><br><span class="subtitle notice-rich-text">${sanitizeHtml(notice.body || '')}</span></span><span class="subtitle">${new Date(notice.created_at).toLocaleDateString('en-BD')}</span></div>`).join('') : '<div class="empty">No notices.</div>';
    ...
    } catch (error) { root.innerHTML = `<div class="card"><h2>Could not load portal</h2><p class="subtitle">${escapeHtml(error?.message || 'Please try again.')}</p></div>`; }
  }
