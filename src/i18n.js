...
(function () {
  ...
  let applyingAuth = false;

  function applyAuth() {
    if (applyingAuth) return;
  ...
  // No MutationObserver on #auth-screen — text updates would re-enter applyAuth and freeze the tab.
  // Auth strings refresh via applyAuth() from renderAuth / lang toggle / applyAll only.
